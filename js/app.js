...
(function () {
  ...
  // Scroll: adds .scrolled to the navbar
  ...
  segBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // Paneles
      segPanels.forEach(p => {
        const isActive = p.id === `seg-${target}`;
  ...
  function animateCounter(el) {
    ...
    function update(now) {
      ...
      const currentLang = localStorage.getItem('biotrack-lang') || 'en_US';
      const locale = currentLang === 'es_419' || currentLang === 'es' ? 'es-419' : 'en-US';
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', e => {
      ...
      const reasonField = contactForm.elements.reason;
      const selectedReason = reasonField.options[reasonField.selectedIndex].text;
      const subject = `BioTrack contact: ${selectedReason}`;
      const body = [
      ...
      window.location.href = `mailto:contacto@biotrack.pe?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    });
  }
  ...
  /* ─── 9. ACTIVE NAV LINK en scroll (highlight) ─── */
